"""Blocklist management API: the pipeline tracks everything and the user
lists what NOT to process. The matching itself lives in
...
@router.get("")
def get_blocklist(manager: Employee = Depends(get_current_employee)):
    return blocklist.load_blocklist(manager.id)


@router.post("/contacts", status_code=201)
def create_blocked_contact(payload: BlockedContactCreate, manager: Employee = Depends(get_current_employee)):
...
def update_blocked_contact(contact_id: str, payload: BlockedContactUpdate, manager: Employee = Depends(get_current_employee)):
...
def delete_blocked_contact(contact_id: str, manager: Employee = Depends(get_current_employee)):
    if not blocklist.delete_blocked_contact(manager.id, contact_id):
        raise HTTPException(404, f"No blocked contact with id '{contact_id}'.")


@router.post("/channels", status_code=201)
def create_blocked_channel(payload: BlockedChannelCreate, manager: Employee = Depends(get_current_employee)):
...
def delete_blocked_channel(channel_id: str, manager: Employee = Depends(get_current_employee)):
