...
regardless of the simulator's sim clock (app/controlplane/ is exempted
from the wall-clock guard, same reasoning as app/integrations/ and
app/projectkb/)."""
import logging
import secrets
...
from app.controlplane.models import get_controlplane_db, Manager, AuthSession
...
def get_current_manager(request: Request, db: DBSession = Depends(get_controlplane_db)) -> Manager:
    ...
    manager = db.get(Manager, session.manager_id)
    if not manager:
